...
class KaEnables(object):
    ...
    def __init__(self, core):
        ...
        self._core._add_cfunc(self._cfuncs, 'ka_hal_read_subsystem_up'      , POINTER(ka_err), [c_void_p, c_int, POINTER(c_byte)])
        self._core._add_cfunc(self._cfuncs, 'ka_hal_read_subsystem_clocked' , POINTER(ka_err), [c_void_p, c_int, POINTER(c_byte)])
        self._core._add_cfunc(self._cfuncs, 'ka_hal_read_subsystem_powered' , POINTER(ka_err), [c_void_p, c_int, POINTER(c_byte)])
        self._core._add_cfunc(self._cfuncs, 'ka_hal_read_subsystem_run'     , POINTER(ka_err), [c_void_p, c_int, POINTER(c_byte)])
    
    # noinspection PyProtectedMember
    def _lib(self):
        return self._core._get_ka()

    # noinspection PyProtectedMember
    def _handle_return_code(self, res):
        self._core._handle_error(res)
