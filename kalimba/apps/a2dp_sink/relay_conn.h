...
// The effect of changing $TWS_ALIGNMENT_DELAY is explained here:
//
// Let's start with a simplified diagram of the system:
//
// AD2DP Source -> Over-the-air-> Encoded data buffer -> Splitter -> Decoder -> TWS Delay -> DAC
//                                                               |
//                                                    over-the-air to TWS Slave
//                                                               |
//                                                               \/
//                                                    encoded data buffer -> Decoder -> Slave TWS Delay -> DAC
//
//
// $TWS_ALIGNMENT_DELAY controls the number of PCM samples kept in the TWS Delay.  The induced latency is defined as $TWS_ALIGNMENT_DELAY/FS.  For example, if $TWS_ALIGNMENT_DELAY // is defined as 16502 and the sample rate is 44.1 kHz, the latency induced by the TWS latency buffer will be 374 ms.
//
// This delay is required to compensate for the latency on the path between the TWS Master and TWS Slave, which exists between the TWS Master's Splitter and TWS Slave's DAC.  The
// strategy is to set the TWS Delay as large as possible to compensate for this transmission delay.  Large TWS Delay values on the TWS Master provide more time for data get
// over-the-air and therefore provide more robust performance compared to smaller delay values.
...
#else
#define $TWS_STALL_TO_DECODE_THRESHOLD_MASTER   150000
#define $TWS_STALL_TO_DECODE_THRESHOLD_SLAVE    300000
#endif
#else // A2DP_SINK_LOWMEM
