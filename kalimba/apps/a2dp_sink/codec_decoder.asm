...
#define        MAX_MACRO(x,y)             ((x)>(y)?(x):(y))
   ...
   DeclarePackedCBuffer($codec_mono_out_left_cbuffer_struc, $codec_mono_out_left, AUDIO_CBUFFER_SIZE/4 + 2);
   DeclarePackedCBuffer($codec_mono_out_right_cbuffer_struc, $codec_mono_out_right, AUDIO_CBUFFER_SIZE/4 + 2);
#endif
#else
   DeclareCBuffer($codec_in_cbuffer_struc,$codec_in,CODEC_CBUFFER_SIZE);
#ifdef APTX_MONO_RELAY
   DeclareCBuffer($codec_mono_out_left_cbuffer_struc, $codec_mono_out_left, AUDIO_CBUFFER_SIZE/4 + 2);
   DeclareCBuffer($codec_mono_out_right_cbuffer_struc, $codec_mono_out_right, AUDIO_CBUFFER_SIZE/4 + 2);
#endif
#endif
...
// option selected and enable application static allocation below
  ...
  // Structure to measure output port consuming rate
...
#ifdef APTX_MONO_RELAY
   // Allocating memory for decoder codec stream struc
   .VAR/DM1 $decoder_mono_codec_stream_struc[$codec.av_decode.STRUC_SIZE] =
      $aptx.decode.mono,                           // frame_decode function (set by conn_init function)
      $aptx.reset_decoder,                         // reset_decoder function (set by conn_init function)
      $aptx.silence_decoder,                       // silence_decoder function (set by conn_init function)
      &$codec_mono_out_right_cbuffer_struc,        // input cbuffer from left/right demux output
      0,             // out left cbuffer
      0,                                           // out right cbuffer
      0 ...;                                       // Pad out remaining items with zeros
#endif
   ...
   .VAR cbuffers_latency_measure[] =
    ...
    //TWS PCM latency measurement
    //Measure latency of all samples which have been warped in SW already
   .VAR tws_cbuffers_latency_measure[] =
    $codec_rate_adj_out_left_cbuffer_struc, $intermediate_fs_1, $latency_calc_port_warp,
    $multi_chan_primary_left_out_cbuffer_struc, $intermediate_fs_1, $latency_calc_port_warp,
    $MULTI_CHAN_PRIMARY_LEFT_OUT_PORT, $intermediate_fs_1, $latency_calc_port_warp,
    0;
    //There are in principle two kinds of samples to which warp has not been applied yet
    //Samples which will use the current warp rate and samples which will use the scheduled warp rate
   .VAR $latency_scheduled_warp_addr = &$tws.scheduled_sra;
   .VAR $latency_synchronised_warp_addr = &$tws.synchronised_sra;
   .VAR tws_samples_latency_measure[] =
    0, &$inv_codec_fs, &$latency_scheduled_warp_addr,
    0, &$inv_codec_fs, &$latency_synchronised_warp_addr,
    0;
    .VAR $tws_pcm_latency_input_struct[$pcm_latency.STRUC_SIZE] =
      &tws_cbuffers_latency_measure,
      &tws_samples_latency_measure;
...
//          averaging routine.)
   ...
   frame_loop:
      ...
      null = M[$tws.mono_relay_flag];
      if NZ call $tws.configure_mono_relay;

      r0 = M[$tws.mono_relay_configured];
      if Z jump not_tws_mono_relay;

	  null = r0 - $TWS_MASTER_MODE;
	  if Z jump tws_master_mode;
	  
	  null = r0 - $TWS_SLAVE_MODE;
	  if NZ jump tws_master_mode;
	  
	  // Device is a TWS Slave decoding a mono signal  
	  r5 = &$relay_struc;
      call $relay.start;
...
#ifdef APTX_MONO_RELAY
      r2 = M[$decoder_codec_stream_struc + $codec.av_decode.MODE_FIELD];
      Null = r2 - $codec.SUCCESS;
      if NZ jump no_advance;

      // advance output buffer that slave mono decoder did not write to
      r8 = $decoder_codec_stream_struc;
      r10 = M[r8 + $codec.av_decode.DECODER_NUM_OUTPUT_SAMPLES_FIELD];  

      r0 = M[$decoder_codec_stream_struc + $codec.av_decode.OUT_RIGHT_BUFFER_FIELD];
      call $cbuffer.advance_write_ptr;
#endif

no_advance:
	  jump skip_silence_insertion;
...
tws_master_mode:
	  ...
	  .VAR $tws.read_address_before_decode;      
      r0 = M[$decoder_mono_codec_stream_struc + $codec.av_decode.IN_BUFFER_FIELD];
      call $cbuffer.get_read_address_and_size;
      M[$tws.read_address_before_decode] = r0;

	  r5 = &$relay_struc;
      call $relay.start;
	  
	  r5 = $decoder_mono_codec_stream_struc;
      if NZ call $codec.av_decode;
      
      // if successful PCM audio will be in the buffer connected to $codec.av_decode.OUT_LEFT_BUFFER_FIELD
      // the $codec.av_decode.OUT_RIGHT_BUFFER_FIELD so this needs to be done manually.
      r0 = M[$decoder_mono_codec_stream_struc + $codec.av_decode.MODE_FIELD];
      Null = r0 - $codec.SUCCESS;
      if NZ jump no_new_output_data;

	  r10 = M[$decoder_mono_codec_stream_struc + $codec.av_decode.DECODER_NUM_OUTPUT_SAMPLES_FIELD];
      r0 = &$audio_out_left_cbuffer_struc;
      r1 = &$audio_out_right_cbuffer_struc;
      r2 = M[$tws.message_routing_mode_slave];
      null = r2 - $TWS_ROUTING_LEFT;
      if NZ r0 = r1;
      call $cbuffer.advance_write_ptr;

	  call $tws.aptx_mono_relay;

no_new_output_data:
      jump skip_silence_insertion;
      
#endif // APTX_MONO_RELAY
#endif // defined(SBC_MONO_RELAY) || defined(APTX_MONO_RELAY)
...
no_va_copy:
   ...
   r8 = M[$multi_chain0_copy_struc_ptr];
   ...
   r8 = M[$multi_chain1_copy_struc_ptr];
   null = M[$M.multi_chan_output.num_chain1_channels];
   if NZ call $cbops.dac_av_copy_m;
...
.MODULE $M.con_in_copy_handler;
   ...
   call $sra_tagtimes;
   #ifdef LATENCY_REPORTING
   call $M.report_latency_to_vm.calculate_average_latency;
   #endif
...
#ifdef TWS_DELAY_ALWAYS_USED
   // Compute TWS delay in microseconds
   r3 = M[$delay_left + $tws.delay.DELAY_FIELD];
   r2 = M[$inv_codec_fs];
   rMAC = r2 * r3;
   rMAC = rMAC ASHIFT 7;
   r2 = rMAC1;
   r2 = r2 * 0.001(frac);
   r0 = r0 - r2; // account for TWS delay line, which will delay the audio   
#endif
