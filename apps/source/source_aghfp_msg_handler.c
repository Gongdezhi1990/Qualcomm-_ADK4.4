...
/******************/
       ...
       /* confirmation of sending HF indicator */
        case AGHFP_SEND_BIND_INDICATOR_STATE_CFM:
        {
            AGHFP_MSG_DEBUG(("AGHFP_SEND_BIND_INDICATOR_STATE_CFM:\n"));  
        }
        return;
        ...
        case AGHFP_NETWORK_OPERATOR_IND:
        {
            ...
            AghfpSendNetworkOperator(((AGHFP_NETWORK_OPERATOR_IND_T *)message)->aghfp, 
                                     ...
                                     aghfp_get_network_operator_instance());
        }
        return;
        ...
        case AGHFP_CALL_INDICATIONS_STATUS_REQUEST_IND:
        {
            ...
            AGHFP_MSG_DEBUG(("    avail[%d] call[%d] call_setup[%d] call_held[%d] signal[%d] roam[%d] batt[%d]\n",
                                                ...
                                                aghfp_get_state_call_setup_status(),
        ...
        return;
         ...
         /*HF indicator reported by HF */
        case AGHFP_HF_INDICATORS_REPORT_IND:
	    {
	        AGHFP_MSG_DEBUG(("AGHFP_HF_INDICATORS_REPORT_IND HF indicator mask = %x\n",
		 	          ((AGHFP_HF_INDICATORS_REPORT_IND_T*)message)->hf_indicators));
        }
	    return;
	
        case AGHFP_HF_INDICATORS_STATUS_REQUEST_IND:
        {
	         AGHFP_MSG_DEBUG(("AGHFP_HF_INDICATORS_STATUS_REQUEST_IND:\n"));
	         AghfpHfIndicatorsInitStatusResponse(((AGHFP_HF_INDICATORS_STATUS_REQUEST_IND_T*)message)->aghfp,
		  	                 aghfp_get_hf_indicator_state());
        }
        return;

	    case AGHFP_HF_INDICATORS_VALUE_IND:
        {
             AGHFP_MSG_DEBUG(("AGHFP_HF_INDICATORS_VALUE_IND: AssignedNum[%d] Value[%d]\n",
                      ((AGHFP_HF_INDICATORS_VALUE_IND_T *)message)->hf_indicator_assigned_num,
                      ((AGHFP_HF_INDICATORS_VALUE_IND_T *)message)->value));                
        }
	    return;
...
/******************/
        ...
        /* indication of remote QTIL supported features */
        ...
        /* indication of QTIL feature negotiation */
        ...
        /* indication of QTIL feature negotiation request */
...
DESCRIPTION
    Handles AGHFP library message AGHFP_INIT_CFM.
...
static void aghfp_msg_init_cfm(const AGHFP_INIT_CFM_T *message)
{
    ...
    {
        /* store AGHFP pointer */
        aghfp_set_instance(message->aghfp);

        /* get CIND requests sent to the application */
        AghfpCindStatusPollClientEnable(aghfp_get_instance(), TRUE);

        if(aghfp_get_profile_Value() == HFP_PROFILE_1_7)
        {
	       /* Enable HF indicators*/
	       AghfpBindIndicatorEnable(aghfp_get_instance(), 
...
/****************************************************************************
NAME    
    aghfp_msg_slc_connect_cfm - 

DESCRIPTION
   Handles AGHFP library message AGHFP_SLC_CONNECT_CFM.
    ...
    switch (aghfp_get_state(inst))
    {
        ...
        case AGHFP_STATE_DISCONNECTED: /* incase a connection crossover failed */
        {
            ...
            {               
                ...
                /* Write APT to controller if link is SC enable*/
                sc_write_apt(inst);
                /* Inform aghfp library the link is secure */
                sc_set_aghfp_link_mode_secure(inst);

                if(is_link_secure(inst))
                {
                   usb_send_device_command_link_mode(USB_DEVICE_DATA_AG_LINK_MODE_SECURE);
                }
                else
                {
                   usb_send_device_command_link_mode(USB_DEVICE_DATA_AG_LINK_MODE_UNSECURE);
                }
            ...
            {            
                ...
                if ((inst->aghfp_support == AGHFP_SUPPORT_YES) && 
                    ...
                    ((inst->aghfp_connection_retries < aghfp_get_max_connection_retries()) ||
                     security_reconnect))
                {
                    ...
                    MessageSendLater(&inst->aghfpTask, AGHFP_INTERNAL_CONNECT_REQ, 0, aghfp_get_connection_failed_timer());                    
...
/****************************************************************************
NAME    
    aghfp_msg_slc_connect_ind - 

DESCRIPTION
   Handles AGHFP library message AGHFP_SLC_CONNECT_IND.
...
static void aghfp_msg_slc_connect_ind(const AGHFP_SLC_CONNECT_IND_T *message)
{
    ...
    AghfpSlcConnectResponse(aghfp_get_instance(), accept);
}


/****************************************************************************
NAME    
    aghfp_msg_slc_disconnect_ind -

DESCRIPTION
    Handles AGHFP library message AGHFP_SLC_DISCONNECT_IND.
...
static void aghfp_msg_slc_disconnect_ind(const AGHFP_SLC_DISCONNECT_IND_T *message)
{
    ...
    switch (aghfp_get_state(inst))
    {
        ...
        {            
            if ((message->status == aghfp_disconnect_link_loss) && (connection_mgr_get_linkloss_reconnect_delay_timer() != TIMER_NO_TIMEOUT))
            {
                AGHFP_MSG_DEBUG(("  Link-Loss\n"));
                MessageSendLater(app_get_instance(), APP_LINKLOSS_IND, 0, D_SEC(connection_mgr_get_linkloss_reconnect_delay_timer()));
            }            
            /* register disconnection with connection manager */
            connection_mgr_set_profile_disconnected(PROFILE_AGHFP, &inst->addr);

            /* update the link mode as link is gone */
            usb_send_device_command_link_mode(USB_DEVICE_DATA_AG_LINK_MODE_NO_SLC_CONNECTION);
...
/****************************************************************************
NAME    
    aghfp_msg_audio_connect_ind -

DESCRIPTION
     Handles AGHFP library message AGHFP_AUDIO_CONNECT_IND.
...
static void aghfp_msg_audio_connect_ind(const AGHFP_AUDIO_CONNECT_IND_T *message)
{
    ...
    {    
        switch (aghfp_get_state(inst))
        {
            ...
            case AGHFP_STATE_CONNECTING_AUDIO_LOCAL:
            {
                ...
                AghfpAudioConnectResponse(inst->aghfp, TRUE, aghfp_source_data_get_syncpkt_types(), aghfp_source_data_get_audio_plugin_params()); 
            }
            return;
    ...
    AghfpAudioConnectResponse(aghfp_get_instance(), FALSE, 0, NULL);
}


/****************************************************************************
NAME    
    aghfp_msg_audio_connect_cfm -

DESCRIPTION
      Handles AGHFP library message AGHFP_AUDIO_CONNECT_CFM.
...
/****************************************************************************
NAME    
    aghfp_msg_audio_disconnect_ind - 

DESCRIPTION
      Handles AGHFP library message AGHFP_AUDIO_DISCONNECT_IND.
...
/****************************************************************************
NAME    
    aghfp_msg_caller_id_setup - 

DESCRIPTION
      Handles AGHFP library message AGHFP_CALLER_ID_SETUP_IND.
...
/****************************************************************************
NAME    
    aghfp_msg_internal_connect_req -

DESCRIPTION
       Handles AGHFP application message to connect a device.
...
/****************************************************************************
NAME    
    aghfp_msg_internal_connect_audio_req - 

DESCRIPTION
       Handles AGHFP application message to connect audio.
...
static void aghfp_msg_internal_connect_audio_req(Task task)
{
    ...
    switch (aghfp_get_state(inst))
    {
        ...
        case AGHFP_STATE_CONNECTED_AUDIO:
        {
            ...
            if ((audio_get_audio_routed() != AUDIO_ROUTED_AGHFP) &&
                (audio_get_voip_music_mode() == AUDIO_VOIP_MODE))
            {  
...
/****************************************************************************
NAME    
    aghfp_msg_internal_disconnect_audio_req -

DESCRIPTION
        Handles AGHFP application message to disconnect audio.
...
/****************************************************************************
NAME    
    aghfp_msg_internal_disconnect_req - 

DESCRIPTION
        Handles AGHFP application message to disconnect connection.
...
/****************************************************************************
NAME    
    aghfp_msg_internal_create_call_req - 

DESCRIPTION
        Creates Call request based on the AGHFP current state.
...
static void aghfp_msg_internal_create_call_req(Task task, const AGHFP_INTERNAL_CALL_CREATE_REQ_T *req)
{
    ...
    switch (aghfp_get_state(inst))
    {
        ...
        {            
            ...
            AghfpCallCreate(aghfp_get_instance(), req->call_type, FALSE);
        }
        break;
...
/****************************************************************************
NAME    
    aghfp_msg_internal_send_ring - 

DESCRIPTION
        Function whch sends the ring indicator for an incoming call.
...
static void aghfp_msg_internal_send_ring(Task task)
{
    ...
    switch (aghfp_get_state(inst))
    {
        ...
        {                    
            ...
            if (aghfp_get_state_call_setup_status() == aghfp_call_setup_incoming)
            {
                AghfpSendRingAlert(aghfp_get_instance());
                /* Send CLI if supplied */
                if (aghfp_get_state_ring() && inst->cli_enable)
                {
                    AghfpSendCallerId(aghfp_get_instance(), 
                                      aghfp_get_state_ring_cliptype(), 
                                      aghfp_get_state_ring_size_clipnumber(), 
                                      aghfp_get_state_ring_clipnumber(), 
                                      0, 
                                      NULL);
                }
            }
