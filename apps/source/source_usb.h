...
typedef enum
{
    ...
    USB_HOST_COMMAND_AG_HF_INDICATOR,
    USB_HOST_COMMAND_AG_LINK_MODE
} USB_HOST_COMMAND_T;
...
RETURNS
    void
*/
void usb_handle_report(MessageId id,
                                                const USB_DEVICE_CLASS_MSG_REPORT_IND_T *msg);
...
DESCRIPTION
    Sends the link mode (Secure or Unsecure link) to the Host
...
#else
#define usb_get_hid_consumer_interface() (FALSE)
#endif
/*************************************************************************
NAME
...
#else
#define usb_get_audioactive_timer() ((uint16)0))
#endif
/*************************************************************************
NAME
