        ...
        {            
            ...
            else
            {
                if ((states_get_state() == SOURCE_STATE_CONNECTED) &&
                    ...
                    (inst->avrcp_connection_retries < avrcp_get_max_connection_retries()))
                {
                    ...
                    MessageSendLater(&inst->avrcpTask, AVRCP_INTERNAL_CONNECT_REQ, 0, avrcp_get_connection_failed_timer());                    
...
static void avrcp_msg_connect_ind(const AVRCP_CONNECT_IND_T *message)
{
    ...
    if (inst == NULL)
    {
        ...
        AvrcpConnectResponse(app_get_instance(), message->connection_id, message->signal_id, accept, NULL);
    }
    else
    {
        AVRCP_MSG_DEBUG_INST(inst);
        ...
        AvrcpConnectResponse(&inst->avrcpTask, message->connection_id, message->signal_id, accept, &(message->bd_addr));
    }
...
static void avrcp_msg_unitinfo_ind(const AVRCP_UNITINFO_IND_T *message)
{
    ...
    switch (avrcp_get_state(inst))
    {
        ...
        {        
            ...
            AvrcpUnitInfoResponse(message->avrcp, TRUE, subunit_panel, 0, avrcp_get_company_id());
        }
        break;
    ...
    switch (avrcp_get_state(inst))
    {
        ...
        {         
           /* check the Vendor AVRCP command is one the dongle recognises */
            if ((message->company_id ==avrcp_get_company_id()) &&(avrcp_get_vendor_enabled()))
            {
        ...
        case AVRCP_STATE_CONNECTED:
        {
            if ((message->response == avctp_response_interim) || (message->response == avctp_response_changed))
            {
                ...
                if (a2dp_get_number_connections() == 1)
                {
                    ...
                    if (volume_get_speaker_volume() < volume)
                    {
