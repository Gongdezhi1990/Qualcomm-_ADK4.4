...
static void processAllBundleFileEntries(const downloadable_caps_config_def_t *config, uint16 max_config_size)
{
    if (strcmp((const char *) config->filename_1, ""))
        processBundleFileEntry((const uint16 *)config->filename_1, PROCESSOR_MAP(config->applicable_processor_type_1), max_config_size);
    if (strcmp((const char *) config->filename_2, ""))
        processBundleFileEntry((const uint16 *)config->filename_2, PROCESSOR_MAP(config->applicable_processor_type_2), max_config_size);
    if (strcmp((const char *) config->filename_3, ""))
        processBundleFileEntry((const uint16 *)config->filename_3, PROCESSOR_MAP(config->applicable_processor_type_3), max_config_size);
    if (strcmp((const char *) config->filename_4, ""))
        processBundleFileEntry((const uint16 *)config->filename_4, PROCESSOR_MAP(config->applicable_processor_type_4), max_config_size);
    if (strcmp((const char *) config->filename_5, ""))
        processBundleFileEntry((const uint16 *)config->filename_5, PROCESSOR_MAP(config->applicable_processor_type_5), max_config_size);
}
...
{
    uint16 config_size;
    const downloadable_caps_config_def_t *config;

    config_size = configManagerGetReadOnlyConfig(DOWNLOADABLE_CAPS_CONFIG_BLK_ID, (const void **) &config);

    if(config_size)
        processAllBundleFileEntries(config, config_size);

    configManagerReleaseConfig(DOWNLOADABLE_CAPS_CONFIG_BLK_ID);
}
