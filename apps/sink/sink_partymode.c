...
{
    if((BdaddrIsSame(getA2dpLinkBdAddr(index), sinkAvrcpGetLinkBdAddr(a2dp_primary))&&(sinkAvrcpGetPlayStatus(a2dp_primary) == (SinkAvrcp_play_status)play_status))||
       (BdaddrIsSame(getA2dpLinkBdAddr(index), sinkAvrcpGetLinkBdAddr(a2dp_secondary))&&(sinkAvrcpGetPlayStatus(a2dp_secondary) == (SinkAvrcp_play_status)play_status)))
    {
        return TRUE;
    }
    else
