...
//------------------------------------------------------------------------------
    ...
    if Z jump dontCalcPriEqCoefs;
        r9 = M[r7 + $spkr_ctrl_system.INIT_RUNTIME_STRUCT_PTR];
        r9 = r9 + $spkr_ctrl_system.CACHE_PRIM_EQ_PROC_FUNC;        // cache prim eq proc function
        r7 = M[r7 + $spkr_ctrl_system.INIT_PRI_EQ_DEFN_PTR];        // primary eq filter definition object
        r0 = M[r7 + $user_eq.EQ_PROC_FUNC];
        M[r9] = r0;                                                 // cache prim eq proc function
    ...
    if Z jump dontCalcSecEqCoefs;
        r9 = M[r7 + $spkr_ctrl_system.INIT_RUNTIME_STRUCT_PTR];
        r9 = r9 + $spkr_ctrl_system.CACHE_SEC_EQ_PROC_FUNC;         // cache sec eq proc function
        r7 = M[r7 + $spkr_ctrl_system.INIT_SEC_EQ_DEFN_PTR];        // secondary eq filter definition object
        r0 = M[r7 + $user_eq.EQ_PROC_FUNC];
        M[r9] = r0;                                                 // cache sec eq proc function
    ...
    if Z jump dontCalcBassEqCoefs;
        r9 = M[r7 + $spkr_ctrl_system.INIT_RUNTIME_STRUCT_PTR];
        r9 = r9 + $spkr_ctrl_system.CACHE_BASS_EQ_PROC_FUNC;        // cache bass eq proc function
        r7 = M[r7 + $spkr_ctrl_system.INIT_BASS_EQ_DEFN_PTR];       // bass eq filter definition object
        r0 = M[r7 + $user_eq.EQ_PROC_FUNC];
        M[r9] = r0;                                                 // cache bass eq proc function
...
//------------------------------------------------------------------------------
    ...
    if Z jump dontFilterBass;
        r9 = M[r7 + $spkr_ctrl_system.CACHE_BASS_EQ_PROC_FUNC];
        pushm <r7,r8>;
        r7 = r0;
    ...
    if z jump dontCreateLeftPriOutput;
        ...
        if Z jump dontFilterLeftPriOutput;
            r9 = M[r7 + $spkr_ctrl_system.CACHE_PRIM_EQ_PROC_FUNC];
            pushm <r7,r8>;
            r7 = r0;
            call r9;
            popm <r7,r8>;
    ...
    if z jump dontCreateLeftSecOutput;
        ...
        if Z jump dontFilterLeftSecOutput;
            r9 = M[r7 + $spkr_ctrl_system.CACHE_SEC_EQ_PROC_FUNC];
            pushm <r7,r8>;
            r7 = r0;
            call r9;
            popm <r7,r8>;
...
//------------------------------------------------------------------------------
    ...
    // copy left and right inputs to primary outputs at full output:
    //--------------------------------------------------------------------------
    // if left_prim_output == NULL                          skip primary copy
    // if right_input == NULL || right_prim_output == NULL  mono primary copy
    // else                                                 stereo primary copy
    ...
    r0 = M[r7 + $spkr_ctrl_system.LEFT_PRI_OUTPUT_PTR];
    if Z jump doneCopyInToOutPri; // if left_pri_output == 0, skip prim copy
        r3 = r10;
        call $frmbuffer.set_frame_size;
        call get_ptrs;
        i1 = r0;
        ...
        r0 = M[r7 + $spkr_ctrl_system.RIGHT_INPUT_PTR];
        if Z jump copyInToOutPriMono; // if right_input == 0, mono prim copy
            call get_ptrs;
            i4 = r0;
            l4 = r1;
    
            r0 = M[r7 + $spkr_ctrl_system.RIGHT_PRI_OUTPUT_PTR];
            if Z jump copyInToOutPriMono; // if right_pri_output == null, mono prim copy
                r3 = r10;
                call $frmbuffer.set_frame_size;
                call get_ptrs;
                i5 = r0;
                l5 = r1; // fall-through to stereo prim copy
                do copyLRinToLRoutLoop;
                    r0 = M[i0,1],       r1 = M[i4,1];
                    M[i1,1] = r0,       M[i5,1] = r1;
                copyLRinToLRoutLoop:
                jump doneCopyInToOutPri;
    
        copyInToOutPriMono:
        do copyLinToLoutLoop; // just copy the left channel
            r0 = M[i0,1]; 
            M[i1,1] = r0;
        copyLinToLoutLoop:
    ...
    // if we have a subd output, fill it with zeros or copy LFE to it
    //--------------------------------------------------------------------------
