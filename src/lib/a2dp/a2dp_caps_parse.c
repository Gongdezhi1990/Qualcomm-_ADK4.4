...
/* Determine if aptX classic, LL, HD and Adaptive codecs are compatible. */
static bool areAptxCodecsCompatible(const uint8 *local_caps, const uint8 *remote_caps, uint8 local_losc, uint8 remote_losc,
                                    uint8 stereo_channel_mode_mask, uint8 sample_rate_mask)
{
    /* check length to prevent read off end of buffer */
    ...
    if ((local_caps[6] & remote_caps[6] & stereo_channel_mode_mask) == 0)
        return FALSE;
