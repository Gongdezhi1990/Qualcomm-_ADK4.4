...
*/
void audioDecodeEnableLatencyReporting (void)
{
    DECODER_t* decoder = CsrA2dpDecoderGetDecoderData();
    if (decoder != NULL)
    {
        ...
        {
             /* If the main app has supplied a latency target then provide it to the DSP app 
             * If the target is 0 then the DSP app will use the buffer fill level based algorithm
             */
        ...
        {   /* DSP is loaded */
            ...
            {   /* Latency reporting enabled and DSP has provided a latency value */
                ...
                *latency = latencyParams->last;
                ...
                PRINT(("OK  estimated=%u latency=%u\n", (unsigned int) *estimated, *latency));
                return TRUE;
            }
        }
    ...
    if (decoder)
    {
        if (result == decoder->stream_relay_mode)
        {
            ...
            {  
                ...
                audioDecoderDspSendSbcEncodeParams(decoder->sbc_encoder_format, decoder->sbc_encoder_bitpool);
...
void audioDecodeHandleDspInternalMessage(A2dpPluginTaskdata* task, uint16 id, Message message)
{
    ...
    bool isMono = (AudioConfigGetRenderingMode()==single_channel_rendering)?TRUE:FALSE;
                ...
                case MUSIC_READY_MSG:
                    {
                        if (decoder)
                        {
                            ...
                            KALIMBA_SEND_MESSAGE(MUSIC_SET_PLUGIN_MSG, getDspVariant(variant), (uint16)isMono, 0, 0);
...
{
    uint16 set_format = format;

    if (AudioConfigGetRenderingMode() == single_channel_rendering)
    {
        set_format |= 0x0100 ;/* force word to 16 bit aligned packets for Mono SBC cases*/
    }
    /* Only set routing mode if DSP is *currently* operating as a TWS Master */
    KALIMBA_SEND_MESSAGE(KALIMBA_MSG_SBCENC_SET_PARAMS, set_format, 0, 0, 0);
...
static void connectAudioAV(Source source, bool content_protection)
{
    ...
    switch (decoder->a2dp_plugin_variant)
    {
        ...
        case APTXHD_DECODER:
            ...
            transform = VmalTransformRtpAptxHdDecode(source, sink);
            break;
