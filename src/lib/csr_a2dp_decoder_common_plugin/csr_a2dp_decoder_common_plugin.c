...
static void handleAudioMessage ( Task task , MessageId id, Message message )
{
    switch (id)
    {
        ...
        case (AUDIO_PLUGIN_UPDATE_LATENCY_SETTINGS_MSG):
        {
            if (IsAudioBusy())
            {
                MessageSendConditionallyOnTask(task, AUDIO_PLUGIN_UPDATE_LATENCY_SETTINGS_MSG, 0, AudioBusyPtr());
            }
            else
            {
                /* send updated latency parameters to the DSP */
                audioDecodeEnableLatencyReporting();
            }
        }
        break;
