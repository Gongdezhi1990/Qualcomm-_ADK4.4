...
void CsrAgAudioPluginConnect( CsrAgAudioPluginTaskData *task, Sink audio_sink , AUDIO_SINK_T sink_type, uint16 volume , uint32 rate , bool stereo , AUDIO_MODE_T mode , const void * params , Task app_task)
{
   ...
   {
       AudioPluginSetMicBiasDrive(CSR_AG_AUDIO->voice_mic_params->mic_a, TRUE);
       if(task->two_mic)
           AudioPluginSetMicBiasDrive(CSR_AG_AUDIO->voice_mic_params->mic_b, TRUE);
   }
...
void CsrAgAudioPluginDisconnect( CsrAgAudioPluginTaskData *task )
{
    ...
    if (CSR_AG_AUDIO->voice_mic_params)
    {
        ...
        AudioPluginSetMicBiasDrive(CSR_AG_AUDIO->voice_mic_params->mic_a, FALSE);
        if(task->two_mic)
            AudioPluginSetMicBiasDrive(CSR_AG_AUDIO->voice_mic_params->mic_b, FALSE);
    }
