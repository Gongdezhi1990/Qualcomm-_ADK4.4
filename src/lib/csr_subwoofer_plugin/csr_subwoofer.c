...
static void sendErrorToClient(void)
{
    ...
    AUDIO_DSP_IND_T * message = PanicUnlessNew(AUDIO_DSP_IND_T);
    ...
    MessageSend(plugin_data->app_task, AUDIO_DSP_IND, message);
}
...
void CsrSubwooferPluginSetSampleRate(uint16 sample_rate)
{
    ...
    if (plugin_data->dsp_set_sample_rate != sample_rate)
    {
        ...
        KALIMBA_SEND_MESSAGE(MESSAGE_SET_MUSIC_MANAGER_SAMPLE_RATE, sample_rate/10, 0,0,0);
        ...
        case MESSAGE_FROM_KALIMBA:
        {
            ...
            switch(msg->id)
            {
                ...
                case MUSIC_PARAMS_LOADED_MSG:
                {
                    ...
                    MessageSend(plugin_data->app_task, AUDIO_DSP_READY_FOR_DATA, 0);
                }
                break;
